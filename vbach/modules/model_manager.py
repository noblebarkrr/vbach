...
from vbach.modules.download_source import download_file
...
# def get_folders(models_dir):
  ...
  #  ]# Устарело, это встроенно в интерйейс
...
def delete_model_name(dir_name):
    model_dir = os.path.join(rvc_models_dir, dir_name)
    if os.path.exists(model_dir):
        try:
            if os.path.isdir(model_dir):
                shutil.rmtree(model_dir)
                return f"[-] Модель {dir_name} успешно удалена!"
        except Exception as e:
            raise gr.Error(f"Ошибка при удалении модели: {str(e)}")
    else:
        return f"[-] Модели {dir_name} не существует"
